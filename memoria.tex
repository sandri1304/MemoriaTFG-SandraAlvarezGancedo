...
\begin{itemize}
	...
	\item \textbf{Product-row}: Fila de tarjetas resumen del producto. Permite mostrar varias tarjetas resumen en una misma fila. Se utiliza en el home de la aplicación, tanto en top ventas como en ofertas, en la vista ``Detalle del producto'' en el apartado Artículos relacionados.
	...
	\item \textbf{Product-detail-client}: Detalle del producto. Permite visualizar la información del producto de manera detallada. Se utiliza cada vez que se pulsa sobre la imagen o el modelo del producto del compenente client-product.
	...
	\item \textbf{Client-categorie}: Tarjeta Subcategoría. Permite visualizar el nombre y la imagen con la que se caracteriza una subcategoría. Se utiliza tantas veces como subcategorias tenga la categoría general. 

	\begin{figure}[H]
	  \centering
	  \includegraphics[width=6cm, keepaspectratio]{img/tarjeta_subcategoria.png}
	  \caption{Ejemplo Tarjeta subcategoria (Componente Product-detail-client).}
	  \label{fig:tarjetaResumen}
	\end{figure}
\end{itemize}
...
Todas las vistas de la parte pública de la tienda tienen dos componentes comunes: \textbf{Header} (cabecera de la página) y \textbf{Footer} (pie de página).

\begin{itemize}
	\item \textbf{Header}: Es una Navbar con enlaces que se adaptan según el usuario esté logueado o no. Si el usuario no está logueado tiene un enlace para ver el carrito y otro para realizar login. Sí, el usuario está logueado contiene un enlace para cerrar sesión, otro para acceder a su información personal, un tercero para ver los pedidos realizados y, por último, el enlace para visualizar el carrito. 

	\item \textbf{Footer}: Pie de página que contiene los enlaces para ver las condiciones de uso, la política de privacidad, las preguntas frecuentes y la página de contacto. A diferencia del \textbf{Header}  los enlaces siempres son los mismos.
\end{itemize}
...
\begin{itemize} 
	...
	\item \textbf{Login}: Página donde el usuario puede registrarse o acceder a su cuenta. Consta de un tab con dos pestañas, uno para el registro y otro para el login. Cada una de estas pestañas es un formulario donde el usuario debe rellenar los datos de acceso (en el caso del login) y los datos para darse de alta (en el caso del registro). En ambos formulario se comprueba que los campos estén rellenos y que los datos sean válidos. Cuando se crea una cuenta la contraseña y la contraseña de confirmación deben coincidir. Además, se informa al usuario de la fortaleza de la misma en caso de ataque por fuerza bruta. Si se produce un error en el registro o en el acceso a la cuenta, se muestra un mensaje en pantalla a través del sistema de avisos, explicado en la sección \ref{sec:sistemaAvisos}. 

	\item \textbf{Olvidaste la contraseña}: Vista que permite restablecer la contraseña. Al email introducido en el input se envía un enlace para restablecer la contraseña. Este enlace en una nueva vista con dos input para rellenar la nueva contraseña y su confirmación. Cuando se pulsa el botón ``Cambiar contraseña'' se actualiza la contraseña.  

	\item \textbf{Home}: Vista principal de la parte pública de la aplicación. Está formada por las categorías generales, un carousel, que sirve de sistema de anuncios, el ``Top ventas'' y ``Ofertas''. Tanto ``Top ventas'' como ``Ofertas'' están desarrollados con los componentes reutilizabes: \textbf{Product-row}, que permite mostrar los productos en filas, y \textbf{client-product}, que permite mostrar las tarjetas resumen del producto. Al pulsar sobre la foto o modelo del producto se navega a la vista \textbf{Detalle del producto}. Si se pulsa el botón comprar se añade un producto a la cesta. Las categorías generales son enlaces que permiten navegar a la vista de la categoría general seleccionada. Este elemento de la vista se convierte en un dropdown cuando el tamaño de la vista es pequeño. 

	\item \textbf{Categoría}: Vista en la que se muestran las subcategorías de una categoría general. Todas las categorías generales tienen un formato de vista similar con la variación del título de la categoría y de las subcategorías que se muestran. Las subcategorías se visualizan a través del componente reutilizable \textbf{client-category}, que no es más que una tarjeta con la foto y el nombre de la subcategoría. Si se pulsa sobre la foto se navega al listado de productos de la subcategoría. 

	\item \textbf{Listado de productos de una subcategoría}: Vista en la que se muestra el listado de los productos de una subcategoría. Este listado está páginado, como máximo se van a mostrar 12 productos por página. Se diseño así para que a la query de busqueda de productos fuese más eficiente y más rapida. Para el listado se ha utilizado Angular Material y para la visualización del producto el componente reutilizable \textbf{client-product}. Los productos se pueden ordenadar por: los más vendidos, los mejores valorados o de más caros a más baratos y viceversa. Además, se puede filtrar por marca (únicamente se muestran las marcas de los productos de la subcategoría seleccionada), por precio(entre un rango de 0 a 1000), por disponibilidad, por oferta y por características. Para implementar el menú de filtros se ha utilizado también Angular Material. Para realizar el filtrado y la ordenación de los productos se ha recurrido al uso de la función \emph{aggregate} de Mongoose, ya que permite ir transformando los datos por etapas hasta conseguir los que se desean y, de esta manera, sólo se hacía una query para la búsqueda. 

	\item \textbf{Detalle del producto}: Vista que permite conocer la información detallada de un producto: Foto, marca, modelo, estrellas, envío(si es gratuito o no), promociones si las tuviese, disponibilidad (la información de la disponibilidad varía de color dependiendo del stock del producto. Si no hay stock se muestra en rojo, si el stock es inferior a 5 se muestra en amarillo y si es mayor se muestra en verde), características y reseñas. Las características y las reseñas se sitúan en un tab, en la parte inferior de la vista. Las características se visualizan con el estilo definido cuando el producto se almacenó en el sistema. Para ello, se ha utilizado la librería $HTML-EDITOR$, que permite definir el estilo del texto transformándolo a HTML. En cuanto a la visualización de las reseñas, los usuarios pueden filtrar las reseñas por el número de estrellas que puso el usuario cuando escribió la valoración. Para escribir la valoración del producto, los usuarios deben hacer login en la tienda online. El dialogo que se abre cuando se pulsa sobre ``Escribe una reseña'' se explica más adelante. 
	El botón comprar añade el producto al carrito. Cuando se añade el producto al carrito, el producto se añade a la variable Cart del \emph{LocalStorage} del navegador, permitiendo mantenar el carrito si se cierra el navegador.

	\item \textbf{Carrito}: Vista que muestra el resumen del carrito y que permite tramitar el pedido. Se visualizan los productos que hay en el carrito, el total de la cesta y la manera de recibir los productos. Cuando hay dos productos del mismo modelo, los dos deben recibirse de la misma forma pero, si los productos tienen distinto modelo se puede recibir uno en casa y el otro ir a recogerlo a la tienda. Los productos se pueden eliminar de la cesta individualmente o colectivamente si se pulsa sobre el botón ``Vaciar cesta''. Cada vez que se elimina el producto del carrito se borra de la variable ``Cart'' del \emph{LocalStorage} del navegador. Para finalizar la compra se pulsa sobre el botón ``Tramitar pedido''.
	...
	\item \textbf{Preguntas frecuentes}: Vista que permite conocer la respuesta de las preguntas más frecuentes formuladas por los usarios. Cuando el usuario ha iniciado sesión puede enviar preguntas al administrador a través del formulario que aparece en esta vista. 
\end{itemize}

Vistas donde el usuario necesita estar logueado para visualizarlas: 

\begin{itemize}
	\item \textbf{Añade una reseña}: Vista que permite publicar la valoración de un producto. Es un formulario con los campos valoración general, título de la reseña, comentario o reseña, recomendación a otros usuarios y valoración del producto en cuanto a calidad precio. Esta valoración se mostrará en la vista \textbf{Detalle del producto}. La valoración general en forma de número de estrellas se suma al computo total de estrellas de producto. Este computo es el que luego se muestra en la pantalla de \textbf{Detalle del producto}.

	\item \textbf{Perfil de usuario}: Vista donde se visualiza la información personal del usuario. Cuando un usuario se registra en la aplicación debe rellenar esta información pero, posteriormente puede realizar modificaciones sobre su perfil. La vista es un formulario que contiene los siguientes campos: nombre, apellidos, email, teléfono, fecha de nacimiento, DNI, dirección de facturación, código postal, municipio,  provincia de la dirección de facturación y la dirección, el código postal. el municipio y la provincia de envío. Todos los campos son obligatorios excepto los del envío que serán obligatorios si la dirección de facturación y de envío son diferentes. Si algun campo no se rellena se mostrará un mensaje de error en la pantalla a través del sistema de aviso, explicado en la sección \ref{sec:sistemaAvisos}. Esta vista se muestra cuando se registra un usuario, cuando el usuario accede a ella a través del enlace de la cabecera y cuando se tramita el pedido, ya que debe comprar los datos antes de finalizar con el pedido. 

	\item \textbf{Pedidos del usuario}: Vista que permite conocer al usuario los pedidos realizados y el estado de cada uno de ellos. Es una tabla en la que se visualiza el identificador del pedido, la fecha de entrada, la fecha aproximada de entrega y el estado. La tabla está páginada y el usuario puede decidir la cantidad de pedidos que desea mostrar por página. Además, se le permite filtrar por identificador de pedido, estado ó fecha de entrada. 

	\item \textbf{Métodos de Pago}: Vista donde el usuario decide como desea pagar su pedido. Se puede elegir entre tarjeta de crédito, Paypal y contra-reembolso. Cuando se selecciona tarjeta de crédito aparece un formulario con el tipo de tarjeta, el nombre del titular, la fecha de expiración, el número de tarjeta de pago y el ccv. Todos los campos deben estar rellenos. Además, se comprueba que el número de tarjeta sea válido para el tipo de tarjeta seleccionado, que el CCV sea de tres posiciones y que la fecha de expiración tenga un formato correcto. Cuando se selecciona el método de pago Paypal aparece el botón ``Paypal pagar'', que permite realizar el pago en el sitio web de Paypal. Cuando se selecciona contra-reembolso únicamente aparece el botón ``Finalizar pedido''. El pedido se finaliza cuando se pulsa sobre el botón ``Finalizar Pedido'' o cuando se realiza el pago en el sitio web de Paypal. Al finalizar el pedido se genera la factura excepto en la opción contra-reembolso que se genera manualmente por el administrador una vez que se cobre el pedido. En tarjeta de crédito no se llega a conectar con los bancos porque para poder acceder a su pasarela de pagos se necesitaban datos de cuentas reales. 
 
\end{itemize}

\textbf{Vistas de la trastienda}

Las vistas de la trastienda tienen un componente \textbf{Header}, que contiene un enlace para cerrar sesión y un indicador con el número de pedidos en estado pendiente. La vista principal de la trastienda es un menú en el lado izquierdo con diferentes secciones y un div que varía dependiendo de la sección seleccionada. Las secciones de la trastienda son: Pedidos, clientes, catálogo, servicios, ofertas, importación y mensajes. Excepto la sección de importación, todas ellas contienen una tabla que muestra la información de cada una de las secciones. Cada una de las tablas se puede exportar a un documento excel, se puede filtrar por determinados campos y ordenadar según le convenga al administrador. En las secciones pedidos, facturas y clientes se puede visualizar la información de cada registro a través de una ventana diálogo que se muestra cuando se pulsa sobre el botón ``Info''. Los campos de estas ventanas están bloqueados excepto en pedidos que tanto el estado como cobrado se puede modificar. Además, en pedidos y facturas no se puede eliminar ni crear registros de la tabla. Clientes permite eliminar registros pero no crearlos. En las tablas de las secciones catálogo  (productos) y ofertas también se puede visualizar la información de un registro a través de una ventana diálogo, pero a diferencia de las otras secciones sus campos no están bloqueados, ya que tanto el producto y la oferta puede sufrir modificaciones. La ventana de información de productos tiene la peculiaridad que en sus campos características y otras se puede añadir texto con estilo propio. Para ello, se ha utilizaso la librería $HTML-EDITOR$, que permite convertir el texto formateado en html, por lo que al visualizarlo en la página \textbf{Detalle del producto} se muestra con los estilos definidos por el administrador. Estas tablas permiten crear y eliminar registros. En las secciones catálogo (marcas y categorías) y servicios, sus tablas no tienen el botón ``Info'' que permite ver la información, pero si se pueden eliminar y crear nuevos registros. En la sección de mensajes se puede ver la información del mensaje a través de la ventana diálogo y, en esta ventana, escribir y enviar la respuesta al usuario. Se puede marcar como leído un corro y se puede eliminar un mensaje de la tabla.

La sección de importación es completamente distinta a las demás secciones, ya que en está sección no se muestra ninguna tabla, si no lo que se muestra son los campos necesarios para realizar la importación y descarga del documento excel que actúa como plantilla. Además, contiene un espacio donde se escriben los errores producidos a la hora de realizar la importación. 
...
\begin{figure}[H]
	  \centering
	  \includegraphics[width=14cm, keepaspectratio]{img/homePaginaPrincipal.png}
...
El usuario para restablecer su contraseña debe pulsar sobre el enlace ``Entrar'' de la barra de navegación. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% BIBLIOGRAFIA %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
