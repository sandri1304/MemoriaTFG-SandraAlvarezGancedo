...
Su uso está permitido tanto  en el lado del servidor con NodeJS, como en el lado del cliente con Angular 2, framework de JavaScript.
...
Para la creacción de los documentos excel, se utiliza la librería de NodeJS $excel4node$. El documento excel se va rellenando con los metadatos de la cabecera y los datos devueltos por la base de datos. En el caso de la plantilla, sólo se rellena la cabecera del documento excel. Este documento se envía en la respuesta al cliente y, éste lo descarga utilizando \emph{saveAs} de la librería de Angular $File-saver$. 

\subsection{Envío de correos} 
\label{sec:nodemailer}

Para el envío de correos se ha utilizado la librería $Nodemailer$ de Node. Lo primero que se hace es crear un objeto con la función \emph{createTransport}. A esta función se le pasan las creendenciales de la cuenta desde la que se va a realizar el envío. Posteriormente, se crea un nuevo objeto con los datos del usuario que recibe el correo, el asunto y el mensaje del correo. Y, por último, se envía el correo utilizando la función \emph{sendMail} de la propia librería. El envío de correos se ha usado en el registro del usuario, en el restablecimiento de la contraseña y en la respuesta de las preguntas de los usuarios al administrador. 
...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{index.html}: Archivo HTML principal de la interfaz de usuario. Es el punto de entrada a la aplicación. En él se carga el componente principal, los estilos generales y los scripts necesarios para el correcto funcionamiento del cliente. 
		...
		\item \textbf{\slash assets}: Directorio donde se almacenan los recursos estáticos como el logo o las imágenes del carousel implementado en la página \textit{home} de la aplicación. 
...
\begin{enumerate}
	...
	\item Clonar el repositirio de \textit{Bitbucket} con el comando:

	\emph{git clone https://sandra13@bitbucket.org\slash sandra13\slash proyecto\_v2.git}

	\item Descargar \textit{NodeJs} y \textit{NPM} desde:

	\textit{https://nodejs.org/es/} 

	\item Instalar \textit{NodeJs}. La versión utilizada en la aplicación es 10.16.13 LTS. Esta versión incluye \textit{NodeJs} y \textit{NPM}.

	\item  Instalar \textit{Nodemon} con el comando: 

	\emph{npm install -g nodemon}

	\item Instalar \textit{Angular-cli} con el comando: 
	...
	//Falta comando de importación de la base de datos. 
\end{enumerate}
	...
	\begin{itemize}
		\item \emph{server.bat}, ejecuta el servidor de \textit{NodeJs}. Utilizado para arrancar el servidor de la aplicación. 
		\item \emph{client.bat}, ejecuta el servidor de \textit{Angular-cli} con la configuración del proxy. Utilizado para arrancar el cliente de la aplicación.
	\end{itemize}
\appendix
...
El usuario puede filtrar los pedidos por identificador del pedido, estado y fecha en la que se realizo el pedido. Además, puede seleccionar los pedidos que quiere mostrar en la tabla. 

\section{Añadir reseña}
\label{sec:aniadirResenia}

El usuario puede escribir reseñas de los productos de la tienda. Pero, para ello, debe haber iniciado sesión (ver sección \ref{sec:inicioSesion}). Una vez que el usuario ha iniciado sesión, debe ir a la vista del detalle del producto y pulsar sobre ``Escriba una reseña''. 
...
Para acceder a las preguntas frecuentes, el usuario debe dirigirse al pie de página de la aplicación y pulsar sobre el enlace ``Preguntas frecuentes''. 
...
\begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/piePagina.png}
	  \caption{Enlace quienes somos.}
	  \label{fig:Enlacequienes}
\end{figure}

Al pulsar sobre el enlace se muestra la página de quienes somos. 
...
En esta página se definen las condiciones de uso. Se explica como deben efecturarse los pedidos, como se realizan los envíos, como hay que hacer las devoluciones y como es la garantía de los productos. 
...
El administrador debe iniciar sesión en la aplicación (ver sección \ref{sec:inicioSesion}) para acceder a la trastienda. Cuando se inicia sesión, el administrador ve la página de pedidos, ya que es la página principal de la trastienda. 
\begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/paginaPrincipalAdmin.png}
	  \caption{Pedidos. Página de inicio de la trastienda}
	  \label{fig:paginaInicioTrastienda}
\end{figure}

\subsection{Pedidos}
\label{sec:pedidos}

Página donde se muestran todos los pedidos realizados por los usuarios ordenados por fecha en la que se hizo el pedido. 

Si el administrador desea cambiar la ordenación debe pulsar sobre el campo de la cabecera por el que quiere ordenar. 

\begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/ordenarPedidos.png}
	  \caption{Ordenación por identificador de pedido.}
	  \label{fig:ordenacionPedidos}
\end{figure}

El administrador puede filtrar los pedidos por: Identificador de pedido, fecha de entrada del pedido, fecha de entrega e identificador del cliente. Para ello, debe rellenar el formulario situado en la parte superior de la vista y pulsar sobre el botón ``Buscar''. Si existen pedidos que coinciden con los datos del formulario se mostrarán en la tabla. Si no la tabla quedará vacía. 

\begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/filtrado_pedidos.png}
	  \caption{Filtrado de los pedidos por identificador de pedido.}
	  \label{fig:filtradoPedidos}
\end{figure}

 En la cabecera de la tabla en la parte derecha hay una barra de herramientas, que permite actualizar la tabla, limpiar los filtros y exportar a Excel. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/herramientasPedidos.png}
	  \caption{Funcionalidades de la tabla: Actulizar, limpiar filtros y exportar a Excel.}
	  \label{fig:filtradoPedidos}
\end{figure}

Si el administrador desea exportar la tabla a un documento Excel debe ir a la barra de herramientas de la tabla y pulsar sobre el botón de ``Exportar Excel''. Al pulsar sobre este botón se descarga el documento generado. 

\begin{figure}[H]
	  \centering
	  \includegraphics[width=12.5cm, keepaspectratio]{img/exportacionTabla.png}
	  \caption{Exportación tabla pedidos.}
	  \label{fig:filtradoPedidos}
\end{figure}

Cada registro de la tabla contiene dos botones, uno para ver el pedido y otro para generar la factura. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/registroPedidos.png}
	  \caption{Iconos información del pedido y generación de factura.}
	  \label{fig:iconosPedidos}
\end{figure}

Para visualizar la información del pedido, el administrador debe pulsar sobre el icono ``Info''. Al pulsar sobre el icono, se abre una ventana diálogo donde se muestra la ficha del pedido. 
 \begin{figure}[H]
	  \centering
...
Desde la ficha del pedido el administrador puede actualizar el estado del pedido (pendiente envío, pendiente recoger, preparando, entregado y finalizado) y el campo cobrado, ya que los pedidos en los que se ha utilizado contrareembolso, como método de pago, no se cobran hasta que no se entrega o recoge el producto. Para ello, debe cambiar los campos y pulsar sobre el botón ``Actualizar''. Al actualizar se cierra la ventana y se actualiza el registro con el cambio realizado. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/actualizacionPedido.png}
	  \caption{Pedido Actualizado.}
	  \label{fig:iconosPedidos}
\end{figure}

Para generar una factura, el administrados debe pulsar sobre el icono ``Generar factura''. Solo puede generar facturas en los pedidos que se ha utilizado contrareembolso como método de pago, ya que con tarjeta bancaria y con Paypal se genera automáticamente. 

\subsection{Clientes}
\label{sec:clientes}

Para acceder a la tabla de clientes, el administrador debe pulsar sobre la sección ``Clientes'' del menú de la izquierda. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=4cm, keepaspectratio]{img/menuClientes.png}
	  \caption{Sección Clientes.}
	  \label{fig:menuClientes}
\end{figure}

Seleccionando clientes, se accede a la tabla donde se muestran todos los usuarios registrados en la tienda online. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/clientes.png}
	  \caption{Sección Clientes.}
	  \label{fig:menuClientes}
\end{figure}

El administrador puede filtrar por: Nombre, apellidos, email, DNI y telefóno. Para filtrar debe rellenar el campo o los campos, del formulario de la parte superior, por los que desea filtrar y pulsar sobre el botón ``Buscar''. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/filtrosClientes.png}
	  \caption{Filtrado clientes.}
	  \label{fig:filtradoClientes}
\end{figure}
 ...
 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/herramientasPedidos.png}
	  \caption{Funcionalidades de la tabla clientes: Actulizar, limpiar filtros y exportar a Excel.}
	  \label{fig:herramientas}
\end{figure}

Para exportar la tabla a excel se realiza como en la tabla pedidos (ver sección \ref{sec:pedidos}).

Cada registro de la tabla contiene dos botones, uno para visualizar la ficha del cliente y otro para eliminar al usuario del sistema.

\begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/iconosClientes.png}
	  \caption{Iconos ficha y borrado de cliente.}
	  \label{fig:iconosClientes}
\end{figure}

Para ver la ficha del cliente, el administrador debe pulsar sobre el icono ``Info''. Al pulsar sobre dicho icono se abre una ventana diálogo con la información del usuario. El administrador no puede actualizar ningún dato del cliente.
...
Para eliminar un cliente, el administrador debe pulsar sobre el icono ``x'' que aparece en cada registro. Al pulsar sobre este icono se abre una ventana de confirmación. Si desea borrar el cliente debe pulsar ``Sí'' y si no pulsar ``No''. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=12cm, keepaspectratio]{img/ventanaEliminacion.png}
	  \caption{Ventana de confirmación para eliminar un cliente.}
	  \label{fig:eliminadoCliente}
\end{figure}

Al pulsar sobre ``Sí'', se elimina el cliente, se cierra la ventana, se muestra un mensaje en pantalla y se actualiza la tabla de clientes. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=12cm, keepaspectratio]{img/eliminacionCliente.png}
	  \caption{Mensaje de aviso al eliminar un cliente.}
...
Para acceder a la tabla de facturas, el administrador debe pulsar sobre la sección ``Pedidos'' y sobre ``Facturas'' del menú de la izquierda.

\begin{figure}[H]
	  \centering
	  \includegraphics[width=4cm, keepaspectratio]{img/seccionFacturas.png}
	  \caption{Sección Facturas.}
	  \label{fig:seccionFacturas}
\end{figure}

Al pulsar sobre facturas, se muestran las facturas generadas ordenadas por el número de factura

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/facturas.png}
	  \caption{Tabla de Facturas.}
	  \label{fig:tablaFacturas}
\end{figure}

Se puede filtrar por número de factura, fecha de facturación y por cliente. Para ello, debe rellenar los campos del formulario de la parte superior y pulsar sobre el botón filtrar. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/filtradoFactura.png}
	  \caption{Filtrado de facturas.}
	  \label{fig:FiltradoFacturas}
\end{figure}

En la cabecera de la tabla, en la parte derecha hay una barra de herramientas que permite actulizar la tabla, eliminar los filtros y exportar la tabla a un documento Excel. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/herramientasPedidos.png}
	  \caption{Funcionalidades de la tabla facturas: Actulizar, limpiar filtros y exportar a Excel.}
	  \label{fig:herramientasFacturas}
\end{figure}

Para exportar la tabla a excel se realiza como en la tabla pedidos (ver sección \ref{sec:pedidos}).

Cada registro de la tabla de facturas tiene dos iconos, uno para visualizar la factura y otro para generar el PDF de la factura. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/iconosFactura.png}
	  \caption{Iconos detalle factura y generación PDF.}
	  \label{fig:iconoFactura}
\end{figure} 

Para ver el detalle de la factura, debe pulsar sobre el icono ``info''. Al pulsar se abre una ventana diálogo donde se muestran los detalles de la factura. 
...
Para generar el PDF de la factura, se debe pulsar sobre el icono ``Generar PDF''. Al pulsar sobre el icono, se abre una nueva ventana del navegador donde se muestra el PDF. El administrador puede descargar y guardar el PDF en su equipo. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=9cm, keepaspectratio]{img/pdf.png}
	  \caption{Factura en PDF.}
	  \label{fig:generacionPdfFactura}
\end{figure} 
