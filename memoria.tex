...
\end{figure}
 ...
 El icono de la derecha permite generar la factura para aquellos pedidos pagados a contrareembolso. El resto de facturas se generan automáticamente cuando el usuario finaliza el pago. 
...
El primer icono de la izquierda permite recargar los datos de la tabla. El icono central limpia los valores de los filtros, cargando de nuevo todos los facturas. El icono de la derecha permite exportar la tabla a un documento Excel. 
...
\end{figure}
 ...
 El icono de la derecha permite descargar el PDF de la factura. Ver figura \ref{fig:factura}.  
...
El icono de la derecha permite eliminar un producto. Antes de borrar el producto, el sistema muestra una ventana de confirmación. Ver figura \ref{fig:eliminadoCliente} 
...
Para acceder a la tabla de productos, el administrador debe pulsar sobre la sección ``Catálogo'' y sobre ``Productos'' del menú de la izquierda.

\begin{figure}[H]
	  \centering
	  \includegraphics[width=4cm, keepaspectratio]{img/seccionProductos.png}
	  \caption{Sección Productos.}
	  \label{fig:seccionProductos}
\end{figure}

Al pulsar sobre productos, se muestran los productos almacenados en el sistema.

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/productos.png}
	  \caption{Tabla de productos.}
	  \label{fig:tablaproductos}
\end{figure}

En la cabecera de la tabla, en la parte derecha hay una barra de herramientas que permite actulizar la tabla, añadir un nuevo producto, limpiar filtros y exportar a excel. 




Se puede filtrar por código de producto, categoría, marca y precio. Para ello, debe rellenar los campos del formulario de la parte superior y pulsar sobre el botón ``Buscar''. Para volver a ver la tabla con todos sus registros, se debe pulsar sobre ``Limpiar filtros''.

La exportación de la tabla al documento excel se realiza como en la tabla pedidos (ver sección \ref{sec:pedidos}).

Cada registro de la tabla de producto tiene dos iconos, uno para visualizar el detalle del producto y otro para eliminar el producto.  

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/iconosClientes.png}
	  \caption{Iconos info y eliminar producto.}
	  \label{fig:iconoProductos}
\end{figure}

Para visualizar el detalle del producto, el administrador debe pulsar sobre el botón ``Info''. Al pulsar sobre este botón, se abre una ventana diálago que permite ver el detalle del producto. Todos los campos del producto se pueden modificar. Para ello, se cambia el campo que se desea modificar y se pulsa sobre el botón ``Actualizar''. Al actualizar, la ventana se cierra y se actuliza la tabla con los cambios. Además, se muestra un mensaje indicando que el producto se ha actualizado. 

\begin{figure} [H]
 \centering
 \subfloat{
    \includegraphics[width=0.5\textwidth]{img/fichaProducto.png}}
  \subfloat{
    \includegraphics[width=0.5\textwidth]{img/detalleProducto2.png}}
 \caption{Ficha del producto.}
 \label{fig:fichaProducto}
\end{figure}
El administrador puede dar formato a las características del producto. De tal forma que este formato luego se ve reflejado en la página \textit{Detalle del producto} que ve el usuario. 

Para añadir un nuevo producto al sistema, se debe pulsar sobre el botón ``Añadir producto'' de la barra de herramientas de la cabecera de la tabla. La ventana que se abre es la misma que para ver el detalle del producto, con la diferencia que los campos están vacíos. Cuando los campos se han rellenado, se pulsa el botón ``Guardar'' para añadir el producto a la base de datos. 

Se puede eliminar un producto del sistema. Para ello, se debe pulsar el icono ``x'' del registro que se desea eliminar. Al pulsar sobre el icono se abre la ventana de confirmación mostrada en la figura \ref{fig:eliminadoCliente}. Cuando se elimina el producto, se cierra la ventana, se muestra un mensaje indicando que el producto se ha eliminado y se actualiza la tabla. 


\subsection{Marcas y categorías}
\label{sec:marcasCategorias}

En esta página se muestra tanto la tabla de marcas como la tabla de categorías.

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=15cm, keepaspectratio]{img/marcasCategorias.png}
	  \caption{Página de marcas y categorías.}
	  \label{fig:tablamarcar}
\end{figure}

Para acceder a esta página, el administrador debe pulsar sobre la sección ``Catálogo'' y sobre ``Marcas y Categorías'' del menú situado a la izquierda. 

\begin{figure}[H]
	  \centering
	  \includegraphics[width=4cm, keepaspectratio]{img/seccionMarcas.png}
	  \caption{Sección Marcas y categorías.}
	  \label{fig:seccionMarcas}
\end{figure}

En las cabeceras de las tablas hay una barra de herramientas que permiten la actualización de la tablas, la creacción de nuevas marcas y categorías, limpiar los filtros y exportar la tabla a Excel. 

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/barraProductos.png}
	  \caption{Funcionalidades de las tablas marcas y categorías: Actulizar, añadir, limpiar filtros y exportar a Excel.}
	  \label{fig:herramientasProductos}
\end{figure}

Cada tabla tiene su propio filtro, aunque ambas tablas se filtran por nombre. Para realizar el filtrado, el administrador debe rellenar ambos formularios y pulsar sobre el botón ``Buscar'' de ambas tablas. Para volver a ver en las tablas todos los registros se deben pulsar el botón ``Limpiar filtros'' de cada tabla. 

La exportación de ambas tablas se realiza de la misma forma que en las secciones anteriores (ver sección \ref{sec:pedidos}).

Cada uno de los registros (en ambas tablas) tiene un icono que permite eliminar una marca o una categoría. Para eliminar, por ejemplo, una categoría se debe pulsar sobre este icono. Al pulsar el icono se abre una ventana de confirmación similar a la que se abre cuando se elimina un cliente (ver figura \ref{fig:eliminadoCliente}).

 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/iconoMarca.png}
	  \caption{Icono eliminar marca y categoría.}
	  \label{fig:iconoMarca}
\end{figure}

Para crear una marca o una categoría se debe pulsar sobre el icono ``+'' de la barra de herramientas de la tabla correspondiente. Al pulsar sobre el icono se abre una ventana en la que se añade el nombre de la marca o categoría y el logo de la marca y la categoría. Para seleccionar el logo se debe pulsar sobre el recuadro que aparece en la ventana. Una vez rellenos los datos, se pulsa sobre el botón ``Guardar''. Al guardarse la marca o la categoría se cierra la ventana, se muestra un mensaje de aviso y se actualiza la tabla correspondiente. 
...
Se puede eliminar un producto del sistema. Para ello, se debe pulsar el icono ``x'' del registro que se desea eliminar. Al pulsar sobre el icono se abre la ventana de confirmación mostrada en la figura \ref{fig:eliminadoCliente}. Cuando se elimina el producto, se cierra la ventana, se muestra un mensaje indicando que el producto se ha eliminado y se actualiza la tabla. 
 ...
 \begin{figure}[H]
	  \centering
	  \includegraphics[width=3.5cm, keepaspectratio]{img/crearMarca.png}
	  \caption{Ventana para añadir.}
	  \label{fig:iconoMarca}
\end{figure}
